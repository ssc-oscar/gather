...
# TODO: 
# - read tokens
# - assign dates from date x to date x + 1
# - do for loop with all keys (1 per day)
# - wait for all processes to complete
# - wait aditional hour to not have any GH keys expire on limit per hour
...
# Ran: tokens_date, ...01, ...02, ...03, ...04, ...05, ...06, ...07, 
#      ...08, ...09, ...10, ...11, ...12, ...13, ...14, ...15, ...16,
#      ...17
# Next: done
for i in {1..31}; do (r=$(head -$i tokens_date_redos|tail -1); echo $r | python3 ghUpdatedRepos.py gh$DT repos  &> ghReposList$(echo $r | cut -d ' ' -f2).updt) & done
...
wget http://git.savannah.gnu.org/cgit --no-check-certificat -O git.savannah.gnu.org.html
